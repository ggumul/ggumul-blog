...
export function ProjectCard({ project, records = [] }: { project: ProjectEntry; records?: WritingEntry[] }) {
  ...
  return (
    <div className="group rounded-[1.35rem] border border-line/80 bg-white/45 px-4 py-5 shadow-[0_16px_36px_rgba(68,49,33,0.03)] transition hover:border-point/45 hover:bg-white/70 md:rounded-[1.6rem] md:px-6 md:py-7">
      <div className="grid gap-5 md:grid-cols-[72px_minmax(0,1fr)_260px] md:gap-8">
        <div className="text-[18px] font-semibold tracking-[-0.05em] text-line transition group-hover:text-point md:text-[30px]">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <Link href={`/projects/${project.slug}`} className="inline-block text-[24px] font-semibold tracking-[-0.045em] leading-none text-text transition group-hover:text-point md:text-[42px]">
              {project.title}
            </Link>
            <p className="max-w-2xl text-[15px] leading-7 text-subtext md:text-[16px] md:leading-8">{project.summary}</p>
          </div>
        </div>

        <div className="space-y-4 rounded-[1.15rem] border border-line/70 bg-background/70 p-3.5 md:rounded-[1.25rem] md:p-5">
