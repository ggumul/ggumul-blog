...
describe('public UI copy cleanup', () => {
  ...
  it('keeps public copy from repeating generic view CTAs and dev-record framing', () => {
    ...
    expect(combined).not.toMatch(/화면 있음|가격 연결 중|다른 항목|폰에서 본 장면|바로 고르기|첫 장면|새 장면|퍼즐 흐름|장보기 흐름|변형 규칙|빈 부분 다시 열기|가격 기준 다시 열기|끊긴 장면\s*→|프로젝트 지도|볼 항목|event 이름|payload 기준/);
