...
describe('content loader', () => {
  ...
  it('builds writing taxonomy and home archive state from the rebuilt post', async () => {
    ...
    expect(taxonomy.tags).toEqual(['선택', '왕도 연대기', '원반', '원판', '이동', '잔해일지', '정찰', '조건', '조작', '카드', '턴', '퍼즐', 'TRPG']);
    ...
    expect(snapshot.traces.map((trace) => trace.slug)).toEqual([
      'hanoi-disk-count-3-to-5',
      'trpg-wastelog-drone-signal-choice',
      'hanoi-two-moves-three-towers',
      'wanderer-11-under-15-not-good-card',
    ...
    expect(sections.timeline.map((trace) => trace.slug)).toEqual([
      'hanoi-disk-count-3-to-5',
      'trpg-wastelog-drone-signal-choice',
      'hanoi-two-moves-three-towers',
      'wanderer-11-under-15-not-good-card',
  ...
  it('keeps project relationships resolvable with the rebuilt Wanderer post attached', async () => {
    ...
    for (const project of projects) {
      ...
      if (project.slug === 'wanderer') {
        ...
        expect(projectRecordMap[project.slug].records.map((post) => post.slug)).toEqual([
          'hanoi-disk-count-3-to-5',
          'hanoi-two-moves-three-towers',
        ]);
      } else if (project.slug === 'trpg') {
        expect(project.relatedPosts).toEqual(['trpg-wastelog-drone-signal-choice', 'trpg-medieval-scout-costs-focus']);
        expect(projectRecordMap[project.slug].records.map((post) => post.slug)).toEqual([
          'trpg-medieval-scout-costs-focus',
          'trpg-wastelog-drone-signal-choice',
        ]);
