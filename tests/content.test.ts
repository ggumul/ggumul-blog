...
describe('content loader', () => {
  ...
  it('publishes rebuilt posts only after runtime evidence and Notion drafts', async () => {
    ...
    expect(posts.map((post) => post.slug)).toEqual([
      'trpg-medieval-scout-costs-focus',
    ...
    const trpgMedieval = posts.find((post) => post.slug === 'trpg-medieval-scout-costs-focus');
    ...
    expect(trpgMedieval?.notionSource.pageId).toBe('35c521c1-5180-8119-a8d9-fed02b2c391b');
    expect(trpgMedieval?.notionSource.url).toBe('https://www.notion.so/TRPG-1-35c521c151808119a8d9fed02b2c391b');
    expect(trpgMedieval?.content).toContain('/media/trpg/2026-05-11/medieval-scout-choice.gif');
    expect(trpgMedieval?.content).toContain('정신 수치가 **3/5**에서 **2/5**');
  ...
  it('builds writing taxonomy and home archive state from the rebuilt post', async () => {
    ...
    expect(taxonomy.tags).toEqual(['선택', '왕도 연대기', '원반', '이동', '잔해일지', '정찰', '조건', '카드', '턴', '퍼즐', 'TRPG']);
    expect(snapshot.latest?.slug).toBe('trpg-medieval-scout-costs-focus');
    expect(snapshot.latestTrace?.slug).toBe('trpg-medieval-scout-costs-focus');
    ...
    expect(sections.latest?.slug).toBe('trpg-medieval-scout-costs-focus');
  ...
  it('keeps project relationships resolvable with the rebuilt Wanderer post attached', async () => {
    ...
    for (const project of projects) {
      ...
      if (project.slug === 'wanderer') {
        ...
        expect(project.relatedPosts).toEqual(['trpg-medieval-scout-costs-focus', 'trpg-wastelog-drone-signal-choice']);
        expect(projectRecordMap[project.slug].records.map((post) => post.slug)).toEqual(['trpg-medieval-scout-costs-focus', 'trpg-wastelog-drone-signal-choice']);
      } else {
